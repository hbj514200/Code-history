...
public class Hua_Rong_Road extends JFrame implements MouseListener, KeyListener, ActionListener {
    ...
    public void init() {
        ...
        String pic[] = {"cao", "guan", "zhang", "liu", "zhou", "huang", "bin", "bin", "bin", "bin"};
        for (int k = 0; k < name.length; k++) {
            person[k] = new Person(k, name[k], pic[k]);
        ...
        person[9].setBounds(154, 204, 50, 50);              //我醉了………………
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_DOWN)  go(man, below);
        if (e.getKeyCode() == KeyEvent.VK_UP)    go(man, above);
        if (e.getKeyCode() == KeyEvent.VK_LEFT)  go(man, left);
        if (e.getKeyCode() == KeyEvent.VK_RIGHT) go(man, right);
    }
    ...
    private void go(Person man, JButton direction) {
        ...
        for (int k = 0; k < 10; k++) {
            ...
            if ((manRect.intersects(personRect)) && (man.number != k)) move = false;
        }
